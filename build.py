    ...
    with sv.managed_session() as sess:
        while True:
            try:
                ...
                #print('Features SP', np.shape(features['sp']))
                #print('Features speaker', np.shape(features['speaker']))
                #print('Features f0', features['f0'])
...
def test():
    ...
    x, y = read('./dataset/bin/*/*/1*001.bin', 32, min_after_dequeue=1024, capacity=2048)
    sv = tf.train.Supervisor()
    with sv.managed_session() as sess:
        for _ in range(200):
            x_, y_ = sess.run([x, y])
    ...
    # ===== Read binary ====
    features = read_whole_features('./dataset/bin/Training Set/SM1/*001.bin')
    ...
    # ==== Direct read =====
    f = './dataset/bin/Training Set/SM1/100001.bin'
